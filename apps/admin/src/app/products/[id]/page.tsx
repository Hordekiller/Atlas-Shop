...
export default function EditProductPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({
        ...
        discountStartAt: product.discountStartAt ? product.discountStartAt.slice(0, 16) : '',
        discountEndAt: product.discountEndAt ? product.discountEndAt.slice(0, 16) : '',
      ...
      if (product.images) setImages(Array.isArray(product.images) ? product.images.map((i: any) => typeof i === 'string' ? { url: i, alt: '' } : i) : []);

      if (product.variants) setVariants(product.variants.map((v: any) => ({
        ...v,
        attributes: typeof v.attributes === 'object' ? v.attributes : {},
        images: Array.isArray(v.images) ? v.images.map((i: any) => typeof i === 'string' ? i : i.url) : [],
      })));

      if (product.attrDefs) setAttrDefs(product.attrDefs.map((a: any) => ({
        ...a,
        values: Array.isArray(a.values) ? a.values : [],
      })));

      if (product.specifications) setSpecifications(product.specifications.map((s: any) => ({ name: s.name, value: s.value })));
    }).catch(console.error)
    ...
    try {
      const payload: any = {
        ...
        variants: variants.map((v) => ({
          ...
          attributes: v.attributes, images: v.images || [],
          isActive: v.isActive ?? true,
        })),
        attrDefs: attrDefs.map((a) => ({
          id: a.id || undefined, name: a.name, values: a.values,
        })),
  ...
  if (loading) return (
    <div className="animate-fade-in">
      <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="h-14 rounded-lg animate-pulse" style={{ background: 'var(--v-bg)' }} />
      ))}</div>
    </div>
  );
  ...
  return (
    <div className="animate-fade-in max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: 'var(--v-text)' }}>ویرایش محصول</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--v-text-secondary)' }}>شناسه: {id}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="v-card space-y-5">
          ...
          <div>
            ...
            <textarea className="v-input" rows={2}
        ...
        <div className="v-card space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" className="v-input"
                value={form.maxOrderQty} onChange={(e) => setForm({ ...form, maxOrderQty: e.target.value })} />
            </div>
          </div>
        </div>
        ...
        <div className="v-card space-y-5">
          ...
          <div>
            ...
            <input type="url" className="v-input" dir="ltr" placeholder="https://..."
        ...
        <div className="v-card space-y-5">
          ...
          <div>
            ...
            <input type="text" className="v-input" placeholder="مثال: پرفروش, جدید, پیشنهاد ویژه"
        ...
        <div className="v-card space-y-5">
          <div className="pb-3 border-b" style={{ borderColor: 'var(--v-border)' }}>
            <h3 className="font-bold text-base flex items-center gap-2">
              <Icon icon="tabler:layers-difference" className="w-4 h-4" style={{ color: 'var(--v-primary)' }} />
              تنوع‌ها (Variants)
            </h3>
          </div>

          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--v-text)' }}>ویژگی‌های تنوع (مثل رنگ، سایز)</label>
            {attrDefs.map((attr, idx) => (
              <div key={attr.id || idx} className="flex gap-2 items-center mb-2">
                <input className="v-input w-1/3" placeholder="نام ویژگی"
                  value={attr.name} onChange={(e) => {
                    const next = [...attrDefs];
                    next[idx] = { ...next[idx], name: e.target.value };
                    setAttrDefs(next);
                  }} />
                <input className="v-input flex-1" placeholder="مقادیر (جداشده با کاما)"
                  value={attr.values.join(', ')} onChange={(e) => {
                    const next = [...attrDefs];
                    next[idx] = { ...next[idx], values: e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean) };
                    setAttrDefs(next);
                  }} />
                <button type="button" onClick={() => setAttrDefs(attrDefs.filter((_, i) => i !== idx))}
                  className="v-btn v-btn-sm" style={{ color: 'var(--v-error)' }}>
                  <Icon icon="tabler:trash" className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
            <button type="button" onClick={() => setAttrDefs([...attrDefs, { name: '', values: [] }])}
          ...
          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--v-text)' }}>مقادیر تنوع</label>
            {variants.map((v, idx) => (
              <div key={v.id || idx} className="border rounded-lg p-3 mb-2" style={{ background: 'var(--v-bg)', borderColor: 'var(--v-border)' }}>
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <input className="v-input" placeholder="نام (مثلا قرمز ۴۲)"
                    value={v.name} onChange={(e) => {
                      const next = [...variants];
                      next[idx] = { ...next[idx], name: e.target.value };
                      setVariants(next);
                    }} />
                  <input className="v-input" placeholder="SKU"
                    value={v.sku || ''} onChange={(e) => {
                      const next = [...variants];
                      next[idx] = { ...next[idx], sku: e.target.value };
                      setVariants(next);
                    }} />
                  <input type="number" className="v-input" placeholder="قیمت (اختیاری)"
                    value={v.price ?? ''} onChange={(e) => {
                      const next = [...variants];
                      next[idx] = { ...next[idx], price: e.target.value ? parseFloat(e.target.value) : null };
                      setVariants(next);
                    }} />
                </div>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <input type="number" className="v-input" placeholder="موجودی"
                    value={v.stock ?? 0} onChange={(e) => {
                      const next = [...variants];
                      next[idx] = { ...next[idx], stock: parseInt(e.target.value) || 0 };
                      setVariants(next);
                    }} />
                  <input className="v-input font-mono text-xs" placeholder='ویژگی‌ها (JSON)'
                    value={JSON.stringify(v.attributes)} onChange={(e) => {
                      const next = [...variants];
                      try { next[idx] = { ...next[idx], attributes: JSON.parse(e.target.value) }; }
                      catch { next[idx] = { ...next[idx], attributes: e.target.value }; }
                      setVariants(next);
                    }} />
                </div>
            ...
            <button type="button" onClick={() => setVariants([...variants, { name: '', sku: '', price: null, stock: 0, attributes: {}, images: [] }])}
              className="v-btn v-btn-secondary v-btn-sm">
              <Icon icon="tabler:plus" className="w-3.5 h-3.5" />
              افزودن تنوع جدید
            </button>
        ...
        <div className="v-card space-y-5">
          <div className="pb-3 border-b" style={{ borderColor: 'var(--v-border)' }}>
            <h3 className="font-bold text-base flex items-center gap-2">
              <Icon icon="tabler:settings" className="w-4 h-4" style={{ color: 'var(--v-primary)' }} />
              وضعیت انتشار
            </h3>
          </div>
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.isActive}
                ...
                className="w-4 h-4" />
              <span style={{ color: 'var(--v-text)' }}>محصول فعال / منتشر شده</span>
            </label>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-4">
          <button type="submit" disabled={saving} className="v-btn v-btn-primary">
            {saving ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> در حال ذخیره...</> : <><Icon icon="tabler:device-floppy" className="w-4 h-4" /> ذخیره تغییرات</>}
          </button>
