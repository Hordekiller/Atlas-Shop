...
export class ProductsService {
  ...
  private async generateUniqueSlug(base: string, excludeId?: number): Promise<string> {
  ...
  private stringifyImages(images: any): string {
    const parsed = this.parseImages(images);
  ...
  private transform(product: any) {
    ...
    if (product.variants) {
      for (const v of product.variants) {
        ...
        if (typeof v.attributes === 'string') { try { v.attributes = JSON.parse(v.attributes); } catch { v.attributes = {}; } }
      }
    }
    if (product.attrDefs) {
      for (const a of product.attrDefs) {
        if (typeof a.values === 'string') { try { a.values = JSON.parse(a.values); } catch { a.values = []; } }
      }
    }
    ...
    if (product.categories) {
      product.categories = product.categories.map((pc: any) => pc.category || pc);
    }
    ...
    const [products, total] = await Promise.all([
      this.prisma.product.findMany({
        ...
        include: {
          ...
          variants: {
            where: { isActive: true },
            select: { id: true, name: true, price: true, stock: true, attributes: true, images: true },
          },
        },
      }),
  ...
  async create(dto: CreateProductDto) {
    ...
    if (dto.barcode) {
      ...
      if (existingBarcode) throw new ConflictException(`Barcode "${dto.barcode}" already exists`);
    }
    ...
    if (discountPercent === undefined && dto.salePrice !== undefined && dto.price > 0) {
      discountPercent = Math.round(((dto.price - dto.salePrice) / dto.price) * 100);
    }

    const { variants: variantsData, attrDefs: attrDefsData, specifications: specsData, categories: extraCategories, images, tags, ...rest } = dto as any;

    const data: any = {
      ...rest,
      slug,
      discountPercent,
      images: this.stringifyImages(images),
    ...
    return this.transform(
      this.prisma.$transaction(async (tx) => {
          ...
          for (const v of variantsData) {
            await tx.productVariant.create({
              data: {
                ...
                images: this.stringifyImages(v.images),
        ...
        if (extraCategories && extraCategories.length > 0) {
          const allCategoryIds = [...new Set([dto.categoryId, ...extraCategories])];
  ...
  async update(id: number, dto: UpdateProductDto) {
    ...
    if (data.slug) {
      ...
      if (existingSlug) throw new ConflictException(`Slug "${data.slug}" already exists`);
    }
    ...
    if (data.sku) {
      ...
      if (existingSku) throw new ConflictException(`SKU "${data.sku}" already exists`);
    }
    ...
    if (data.barcode) {
      ...
      if (existingBarcode) throw new ConflictException(`Barcode "${data.barcode}" already exists`);
    }

    if (images !== undefined) {
      data.images = this.stringifyImages(images);
    }
    ...
    // Validate salePrice < price
    const price = data.price ?? (await this.prisma.product.findUnique({ where: { id }, select: { price: true } }))?.price;
    ...
    if (data.price && data.salePrice) {
      data.discountPercent = Math.round(((data.price - data.salePrice) / data.price) * 100);
    }
    ...
    return this.transform(
      this.prisma.$transaction(async (tx) => {
        ...
        // Update variants
        if (variantsData) {
          const existingIds = (await tx.productVariant.findMany({
            where: { productId: id }, select: { id: true },
          })).map((v) => v.id);
          const incomingIds = variantsData.filter((v: any) => v.id).map((v: any) => v.id);
        ...
        if (extraCategories) {
          ...
          const allCategoryIds = [...new Set([dto.categoryId || (await tx.product.findUnique({ where: { id }, select: { categoryId: true } }))!.categoryId, ...extraCategories])];
          await tx.productCategory.createMany({
            data: allCategoryIds.map((catId: number) => ({
              productId: id,
              categoryId: catId,
