...
export class CacheInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const publicPaths = ['/api/products', '/api/categories', '/api/slides/active', '/api/settings/public'];

    const isPublicGet = publicPaths.some((p) => request.path.startsWith(p)) && request.method === 'GET';
