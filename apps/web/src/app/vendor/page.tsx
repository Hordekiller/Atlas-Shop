...
const exampleStats = { products: 24, orders: 156, revenue: 85200000, pending: 8 };

const exampleOrders = [
  { id: 1001, customer: 'علی محمدی', date: '۱۴۰۵/۰۳/۱۰', total: 1250000, status: 'pending' },
  { id: 1002, customer: 'سارا احمدی', date: '۱۴۰۵/۰۳/۱۰', total: 890000, status: 'processing' },
  { id: 1003, customer: 'رضا کریمی', date: '۱۴۰۵/۰۳/۰۹', total: 2450000, status: 'completed' },
  { id: 1004, customer: 'مریم حسینی', date: '۱۴۰۵/۰۳/۰۹', total: 560000, status: 'completed' },
  { id: 1005, customer: 'حسین رضایی', date: '۱۴۰۵/۰۳/۰۸', total: 1780000, status: 'cancelled' },
];
...
export default function VendorPage() {
  ...
  useEffect(() => {
    api.get<{ products: number; orders: number; revenue: number; pending: number }>('/vendor/stats')
      .then(setStats)
      ...
      <div className="dk-container pt-4 pb-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="dk-card p-5 animate-fade-in">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-[var(--dk-text-light)]">محصولات</span>
              <span className="w-10 h-10 rounded-xl flex items-center justify-center text-lg" style={{ background: 'var(--dk-bg)', color: 'var(--dk-primary)' }}>📦</span>
            </div>
          ...
          <div className="dk-card p-5 animate-fade-in">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-[var(--dk-text-light)]">سفارشات</span>
              <span className="w-10 h-10 rounded-xl flex items-center justify-center text-lg" style={{ background: 'var(--dk-bg)', color: 'var(--dk-primary)' }}>📋</span>
            </div>
          ...
          <div className="dk-card p-5 animate-fade-in">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-[var(--dk-text-light)]">درآمد کل</span>
              <span className="w-10 h-10 rounded-xl flex items-center justify-center text-lg" style={{ background: 'var(--dk-bg)', color: 'var(--dk-primary)' }}>💰</span>
            </div>
            <p className="text-2xl font-bold">{loading ? '...' : stats.revenue.toLocaleString()} <span className="text-sm font-normal text-[var(--dk-text-light)]">تومان</span></p>
          </div>

          <div className="dk-card p-5 animate-fade-in">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-[var(--dk-text-light)]">سفارشات در انتظار</span>
              <span className="w-10 h-10 rounded-xl flex items-center justify-center text-lg" style={{ background: 'var(--dk-bg)', color: 'var(--dk-primary)' }}>⏳</span>
            </div>
            <p className="text-3xl font-bold" style={{ color: 'var(--dk-primary)' }}>{loading ? '...' : stats.pending}</p>
          </div>
        </div>
        ...
        <div className="dk-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/vendor/orders" className="text-sm hover:underline" style={{ color: 'var(--dk-primary)' }}>
              مشاهده همه سفارشات
            </Link>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--dk-border)]">
                  <th className="text-right py-3 px-4 font-medium text-[var(--dk-text-light)]">#</th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--dk-text-light)]">مشتری</th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--dk-text-light)]">تاریخ</th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--dk-text-light)]">مبلغ</th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--dk-text-light)]">وضعیت</th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--dk-text-light)]">عملیات</th>
                </tr>
              </thead>
              <tbody>
                {orders.map((order) => (
                  <tr key={order.id} className="border-b border-[var(--dk-border)] hover:bg-[var(--dk-bg)] transition">
                    <td className="py-3 px-4">{order.id}</td>
                    <td className="py-3 px-4">{order.customer}</td>
                    <td className="py-3 px-4 text-[var(--dk-text-light)]">{order.date}</td>
                    <td className="py-3 px-4 font-medium">{order.total.toLocaleString()} تومان</td>
                    <td className="py-3 px-4">
                      <span className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full ${statusBadge[order.status].cls}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
            <div>
              ...
              <ul className="space-y-2 text-xs text-[var(--dk-text-light)]">
                <li><a href="/about" className="hover:text-[var(--dk-primary)]">درباره ما</a></li>
                <li><a href="/contact" className="hover:text-[var(--dk-primary)]">تماس با ما</a></li>
            ...
            <div>
              ...
              <ul className="space-y-2 text-xs text-[var(--dk-text-light)]">
                ...
                <li><a href="/rules" className="hover:text-[var(--dk-primary)]">شرایط بازگشت</a></li>
