...
interface CategoryData {
  ...
  image?: string; children?: { id: number; name: string; slug: string }[];
}
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      api.get<{ data: Product[]; total: number }>(`/products?categoryId=${id}&sort=${sort}&page=${page}&take=${perPage}`),
    ...
    <>
      ...
      <div className="dk-container py-6">
        ...
        <nav className="text-xs text-[var(--dk-text-light)] mb-4">
          ...
          <span className="mx-1.5">/</span>
          <span className="text-[var(--dk-text)]">{cat?.name || 'دسته‌بندی'}</span>
        </nav>
        ...
        <div className="dk-card p-5 mb-5">
          <div className="flex items-center gap-4">
            {cat?.image && (
              <img src={`http://localhost:8000${cat.image}`} alt={cat.name} className="w-20 h-20 rounded-xl object-cover" />
            )}
            <div>
              <h1 className="text-xl font-bold">{cat?.name || 'دسته‌بندی'}</h1>
              {cat?.description && <p className="text-sm text-[var(--dk-text-light)] mt-1">{cat.description}</p>}
            </div>
          </div>
        </div>
